...
function maybeWarnConfigDrift(
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run \`synarcx update\` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global synarcx configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "synarcx config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: synarcx config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `synarcx config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
        ...
        console.log('Config updated. Run `synarcx update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `synarcx config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        if (fs.existsSync(openspecDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx synarcx update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `synarcx update` in your other projects to apply.');
            } catch {
              console.error('`synarcx update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
