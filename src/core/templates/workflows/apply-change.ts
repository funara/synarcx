...
export function getSynApplySkillTemplate(): SkillTemplate {
  return {
    ...
    instructions: `## ⛔ ABSOLUTE RULE — STRICT EXECUTION BOUNDARY ⛔

This command is STRICTLY scoped to implementing tasks explicitly listed in \`tasks.md\`.
The following actions are FORBIDDEN:
- Implementing features or requirements NOT listed in the tasks
- Performing unsolicited refactoring
- Modifying files unrelated to the current task
- Deviating from the \`design.md\` or \`specs\`

BEFORE EVERY TASK: Check — "Is this change explicitly required by the current task?"
If NO → STOP. Tell the user: "This change falls outside the current task scope. We should update the artifacts first if this is needed."
...
export function getSynApplyCommandTemplate(): CommandTemplate {
  return commandFromSkill(getSynApplySkillTemplate(), {
    ...
    content: `## ⛔ ABSOLUTE RULE — STRICT EXECUTION BOUNDARY ⛔
