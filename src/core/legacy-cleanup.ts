...
export interface LegacyDetectionResult {
  ...
  /** Old opp-* skill directories found (from previous opp: prefix init runs) */
  oppSkillDirs: string[];
 ...
 */
async function detectOppSkillDirs(projectPath: string): Promise<string[]> {
  const found: string[] = [];
  ...
  for (const tool of AI_TOOLS) {
    ...
    if (!(await FileSystemUtils.directoryExists(skillsDir))) continue;
